...
int main() {
	...
	vector<vector<unordered_set<int> > > k_map(H, vector<unordered_set<int> >(W));
	for (int h = 0; h < H; h++) {
		for (int w = 0; w < W; w++) {
			scanf("%d", &(map[h][w]));
		}
	...
	while ((pos.empty() == false)){
		...
		if (now_k > 0) {
			for (int d = 0; d < HORSE_MOV_CNT; d++) {
				...
				if ((map[next_pos.first][next_pos.second] == -1) &&
					(k_map[next_pos.first][next_pos.second].find(now_k - 1) != k_map[next_pos.first][next_pos.second].end())) {
					continue;
				}
				...
				cnt.push(now_cnt + 1);
				map[next_pos.first][next_pos.second] = -1;
				k_map[next_pos.first][next_pos.second].insert(now_k - 1);
			}
		}
		...
		for (int d = 0; d < MONKEY_MOV_CNT; d++) {
			...
			if ((map[next_pos.first][next_pos.second] == -1) &&
				(k_map[next_pos.first][next_pos.second].find(now_k) != k_map[next_pos.first][next_pos.second].end())) {
				continue;
			}
			...
			cnt.push(now_cnt + 1);
			map[next_pos.first][next_pos.second] = -1;
			k_map[next_pos.first][next_pos.second].insert(now_k);
		}
